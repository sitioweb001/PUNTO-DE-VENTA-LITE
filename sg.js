// ═══════════════════════════════════════════════════════════════
// ERP POS LITE v3.5 — BACKEND UNIFICADO (Nsg.js)
// ═══════════════════════════════════════════════════════════════
...
const HDR = {
  ...
  Ventas:        ["id","fecha","subtotal","descuento","impuesto","total","pago_con","cambio","usuario","estado"],
...
function registrarVenta(data) {
  ...
  const total = subtotal - (parseFloat(data.descuento)||0);
  const ventaId = uid();
  shV.appendRow([ventaId, new Date(), subtotal, data.descuento||0, 0, total, data.pago_con||0, data.cambio||0, data.usuario||'Sistema', 'completada']);
  for (const item of data.items) {
...
function eliminarEntidad(hoja, data, tipo) {
  ...
  if (idx > -1) { hojaObj.deleteRow(idx+1); return {status:'success', message:`${tipo} eliminado con éxito.`}; }
  return {status:'error', message:'No encontrado.'};
}
